...
class SegmentTree:
    ...
    def build(self):
        self.build_tree(left=0, right=self.len - 1, par=0)
        
    def build_tree(self, left, right, par):
        if left == right:
            self.tree[par] = self.src[left]
            return
        div = (left + right) // 2
        self.build_tree(left, div, (par << 1) + 1)
        self.build_tree(div + 1, right, (par << 1) + 2)
...
if __name__ == '__main__':
    ...
    ARRAY = list(map(int, input('Enter space separated number\n').strip().split(' ')))
    sg = SegmentTree(ARRAY)
    sg.build()

    n_query = int(input('Enter number of queries: '))
    print('Query format')
    print('q low high (to find minimum in range e.g: q 0 3)')
    print('u low high value (to add "value" to all elements e.g: u 3 4 99)')
